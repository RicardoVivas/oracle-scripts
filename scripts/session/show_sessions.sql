rem - Show Sessions information -------------
rem - I found not very useful when used for displaying current sql
...
--List all sessions whose username is not null
...
begin
    ...
    ( select username||'('||sid||','||serial#||') ospid = ' ||  process ||' program = ' || program username,
             ...
             LAST_CALL_ET
        from v$session
        where status = 'ACTIVE'      and rawtohex(sql_address) <> '00'     and username is not null order by last_call_et )
    loop
...
column client_info format a40 word_wrapped
