rem 
rem  Script to investigate a b-tree index structure [ID 989186.1]
rem  
rem  This script will verify the structure of a b-tree index based on the existing table and index statistics.  
rem  The script calculates the following items:
rem  - Estimate the size the index should be. Optimal packing can be specified
rem  - The index layout
rem  
rem  exec index_util.inspect_schema ('SCOTT'); 
rem  select owner, index_name, last_inspected, leaf_blocks, target_size from index_log
rem 
