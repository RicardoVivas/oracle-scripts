Rem
Rem $Header: rdbms/demo/strmmvp2.sql /st_rdbms_11.2.0/2 2011/08/25 12:29:37 snalla Exp $
Rem
Rem strmmvp2.sql
Rem
...
Rem    juyuan      08/03/10 - bug-9950518: fix sql injection
...
begin
  ...
  if (null_check) then
    ...
    tab_cond := tab_cond ||'.' || streams_mv_refresh.rowid_col || 
                  ' = stmv_rid.rid ';
    tab_cond := tab_cond || 
                'and stmv_rid.table_owner = ' || dbms_assert.enquote_literal( 
                 REPLACE(get_mvs()(mv_no).base_tabs(tab_no).table_owner,
                         '''', '''''')) ||
                ' and stmv_rid.table_name  = ' || dbms_assert.enquote_literal(
                  REPLACE(get_mvs()(mv_no).base_tabs(tab_no).table_name,
                          '''', '''''')) || ' ';
...
begin
  ...
  if (streams_mv_refresh.txn_mvs.count > 0) then
    ...
    str := 
      'begin 
         dbms_mview.refresh(list=>' || dbms_assert.enquote_literal(
                               REPLACE(mvlist, '''', '''''')) || ',' ||
...
create or replace procedure verify_tables(tab1 in varchar2, 
                                          tab2 in varchar2, 
                                          max_time in integer default  180) 
...
begin
  ...
  tab1_sql := 'select *  from ' || dbms_assert.qualified_sql_name(tab1);

  tab2_sql := 'select *  from ' || dbms_assert.qualified_sql_name(tab2);
