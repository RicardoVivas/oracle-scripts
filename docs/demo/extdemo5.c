#ifdef RCSID
static char *RCSid =
   "$Header: rdbms/demo/extdemo5.c /st_rdbms_11.2.0/1 2010/07/21 11:35:42 schakrab Exp $ ";
#endif /* RCSID */
   ...
   ******************************/
   if (ix_ind->IndexPartition == OCI_IND_NULL)
     (void)sprintf(updstmt,
                   "UPDATE \"%s\".\"%s_sbtree\" SET f1 = :newval \
                   WHERE f1 = :oldval",
                   OCIStringPtr(envhp, ix->IndexSchema),
                   OCIStringPtr(envhp, ix->IndexName));
   else
     (void)sprintf(updstmt,
                 "UPDATE \"%s\".\"%s_%s_sbtree\" SET f1 = :newval \
                  WHERE f1 = :oldval",
                   OCIStringPtr(envhp, ix->IndexSchema),
                   OCIStringPtr(envhp, ix->IndexName),
                   OCIStringPtr(envhp, ix->IndexPartition));
...
ODCIEnv_ind       *env_ind;
{
  ...
  if (ix_ind->IndexPartition == OCI_IND_NULL)
    (void)sprintf(selstmt, "select f2 from \"%s\".\"%s_sbtree\" \
                  where f1 %s :val",
                  OCIStringPtr(envhp, ix->IndexSchema),
                  OCIStringPtr(envhp, ix->IndexName), relop);
  else
    (void)sprintf(selstmt, "select f2 from \"%s\".\"%s_%s_sbtree\" \
                  where f1 %s :val",
                  OCIStringPtr(envhp, ix->IndexSchema),
                  OCIStringPtr(envhp, ix->IndexName),
