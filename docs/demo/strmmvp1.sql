Rem
Rem $Header: rdbms/demo/strmmvp1.sql /st_rdbms_11.2.0/2 2011/08/25 12:29:37 snalla Exp $
Rem
Rem strmmvp1.sql
Rem
...
begin
  ...
  for i in async_mv_pkg.tab_owners.first..async_mv_pkg.tab_owners.last loop
    ...
    ownerlist := ownerlist || dbms_assert.enquote_literal(
                 REPLACE(async_mv_pkg.tab_owners(i), '''', ''''''));
    namelist  := namelist  || dbms_assert.enquote_literal(
                 REPLACE(async_mv_pkg.tab_names(i), '''', ''''''));
  end loop;
